        ...
        {navData.map((link, index) =>{
          ...
          >
            ...
            <div className='absolute pr-14 right-0 hidden xl:group-hover:flex'>
              <div className='bg-slate-400 font-bold rounded-3xl relative flex text-primary items-center p-[6px]' >
                <div className='text-[12px] leading-none font-semibold capitalize'>
                  {link.name}
                </div>
                {/* triangle */}
                <div className='border-solid border-l-slate-300 border-l-8 border-y-transparent border-y-[6px] rounded-s-lg border-r-0 absolute -right-2'></div>
              </div>
            </div> 
            {/*icon*/}
            <div>{link.icon}</div>
